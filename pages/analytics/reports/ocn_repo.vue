  ...
  <div>
    ...
    <modal name="modal-content" height="auto">
      ...
      <table class="mx-auto">
        ...
        <tr>
          <td class="pr-2 py-2 font-semibold text-gray-600" for="method">
            手段
          </td>
          <td>
            <input
...
export default {
  ...
  middleware({ store }) {
      store.dispatch('getRepoData', {career_type: "ocn"})
  },
  methods: {
    setRepoData() {
    //   var today = new Date();
    ...
    //   var month = today.getMonth() + 1;
      ...
      var ocn_repo_col = firestore().doc(
        `users/${current_user.uid}/ocn_repo/${self.edit_data.doc_id}`
      );

    ocn_repo_col.update({
            shop: self.edit_data.shop,
            attend: self.edit_data.attend,
            method: self.edit_data.method,
    ...
    editRow(row) {
        ...
        (this.edit_data.method = row.method),
    ...
    edit_comit() {
      ...
      if (yesno) {
        //   ※※※※ここ書き換える
        this.$store.commit("editRepoData", {edit_data: this.edit_data, career_type: "ocn"});
    ...
    getReportData() {
        this.rows = this.$store.state.ocn_reports;
    },
    download() {
      // ラベル
      this.labels = {
  ...
  created() {
    // this.$store.dispatch('getRepoData', {career_type: "ocn"})
      // this.getReportData();
  },
  mounted() {
    this.getReportData();
    ...
    beforeDestroy() {
    this.$store.commit('resetRepoData', {career_type: "ocn"})
  }
};
</script>
