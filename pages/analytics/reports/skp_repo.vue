...
export default {
  ...
  // middleware({ store }) {
  //     store.dispatch('getRepoData', {career_type: "skp"})
  // },
  methods: {
    setRepoData() {
    //   var today = new Date();
    ...
    edit_comit() {
      ...
      if (yesno) {
        //   ※※※※ここ書き換える
        this.$store.commit("editRepoData", {edit_data: this.edit_data, career_type: "skp"});
    ...
    getReportData() {
        this.rows = this.$store.state.skp_reports;
    },
    download() {
      // ラベル
      this.labels = {
  ...
  created() {
    this.$store.dispatch('getRepoData', {career_type: "skp"})
    this.getReportData();
  },
  mounted() {
    // this.getReportData();
  ...
  beforeDestroy() {
    this.$store.commit('resetRepoData', {career_type: "skp"})
  }
};
</script>
