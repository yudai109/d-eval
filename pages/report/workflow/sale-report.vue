<template>
    <div class="nipo_conatiner bg-white border rounded-lg py-2 mt-4 shadow-lg">
        ...
        <h1 class="text-3xl pb-2">【報告書】店舗販売支援</h1>
        <p>※店舗稼働終了時に報告</p>
        <p>※複数店舗ラウンドの場合は店舗ごとに報告</p>
        <hr class="my-3">
        <div class="mb-4">
    ...
    <!-- 店舗の当日目標 -->
        <h2 class="text-left pl-4 pt-6 pb-3">店舗の当日目標</h2>
        <hr class="w-11/12 ml-4 mb-4 ">

        <div class="mb-4">
            <label class="label">総販 <span class="bg-red-500 rounded-lg px-2 text-xs text-white">必須</span></label>
            <input v-model="souhan_mokuhyou" type="number" name="souhan" class="border rounded-lg p-2 w-11/12">
        </div>

        <div class="mb-4">
            <label class="label">HS新規 <span class="bg-red-500 rounded-lg px-2 text-xs text-white">必須</span></label>
            <input v-model="hs_mokuhyou" type="number" name="hs_shinki" class="border rounded-lg p-2 w-11/12">
        </div>
    ...
    <!-- マーケティング -->
        ...
        <div class="mb-4">
            <label class="label">来店組数／当日 <span class="bg-red-500 rounded-lg px-2 text-xs text-white">必須</span></label>
            <input v-model="raikyaku_daily" type="number" name="raikyaku_daily" class="border rounded-lg p-2 w-11/12">
        </div>

        <div class="mb-4">
            <label class="label">来店組数／当月累計 <span class="bg-red-500 rounded-lg px-2 text-xs text-white">必須</span></label>
            <input v-model="raikyaku_monthly" type="number" name="raikyaku_monthly" class="border rounded-lg p-2 w-11/12">
        </div>
...
export default {
    ...
    methods: {
        PushNipo: function() {
            ...
            var usersNipoCol = firestore().collection(`users/${CurrentUser.uid}/sale_report`)
            ...
            usersNipoCol.add({
                ...
                souhan_mokuhyou: Number(self.souhan_mokuhyou),
                hs_mokuhyou: Number(self.hs_mokuhyou),
                ...
                .then(function(qs) {
                    qs.forEach(function(doc) {
                        ...
                        .then(function() {
                            PerformanceDoc.set({
                                createdAt: firestore.FieldValue.serverTimestamp(),
                                souhan_mokuhyou: firestore.FieldValue.increment(Number(self.souhan_mokuhyou)),
                                hs_mokuhyou: firestore.FieldValue.increment(Number(self.hs_mokuhyou)),
