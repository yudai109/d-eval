<template>
    <div class="nipo_conatiner bg-white border rounded-lg py-2 mt-4 shadow-lg">
        ...
        <h1 class="text-3xl pb-2">【報告書】外販イベント(参加者)</h1>
        ...
        <div class="mb-4">
            ...
            <input v-model="place" type="text" name="place" class="border rounded-lg p-2 w-11/12 ease-linear transition-all duration-150">
        </div>

        <div class="mb-4">
            <label class="label">開始時間 <span class="bg-red-500 rounded-lg px-2 text-xs text-white">必須</span></label>
            <input v-model="start_time" type="text" name="start_time" class="border rounded-lg p-2 w-11/12 ease-linear transition-all duration-150">
        </div>

        <div class="mb-4">
            <label class="label">終了時間 <span class="bg-red-500 rounded-lg px-2 text-xs text-white">必須</span></label>
            <input v-model="end_time" type="text" name="end_time" class="border rounded-lg p-2 w-11/12 ease-linear transition-all duration-150">
        
        </div>
        <div class="mb-4">
            <label class="label">所感 <span class="bg-red-500 rounded-lg px-2 text-xs text-white">必須</span></label>
            <input v-model="other" type="text" name="other" class="border rounded-lg p-2 w-11/12 ease-linear transition-all duration-150">
        </div>
        <div class="my-3">
...
export default {
    ...
    methods: {
        PushNipo: function() {
            ...
            var usersSkpCol = firestore().collection(`users/${CurrentUser.uid}/event_attend`)
            usersSkpCol.add({
